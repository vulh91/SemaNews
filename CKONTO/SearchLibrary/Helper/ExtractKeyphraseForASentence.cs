...
{
    public class ExtractKeyphraseForASentence
    {
        ...
        public List<POSEntity> SegmenAndPOSTagging(String pStr)
        {
            ...
            {
                anWord = wordList[pos];
                List<POSEntity> inPOSdic = (from pp in SemanticSearch.POSDictionaries
                                            join cc in SemanticSearch.PartOfSpeeches on pp.PartOfSpeechId equals cc.Id
                ...
                List<string> inPhrase = (from p in SemanticSearch.Phrases where p.Name.IndexOf(anWord) == 0 select p.Name.Trim()).ToList();
                if (inPhrase.Count > 0)
                {
                ...
                List<ExtractKeyphraseEntity> inKeyphrase = (from p in SemanticSearch.Keyphrases
        ...
        bool CheckFormedKeyphrase_FromNewKeyphrases(string word, ref POSEntity currentKeyphrase)
        {
            ...
            var isExist = SemanticSearch.Keyphrases.Where(p => String.Compare(p.Name, word) == 0).FirstOrDefault();
