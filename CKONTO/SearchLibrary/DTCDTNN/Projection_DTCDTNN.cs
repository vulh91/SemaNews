...
{
    public class Projection_DTCDTNN
    {
            ...
            else
            {
                for (int i = 0; i < H.relationList.Count; i++)
                {
                    ...
                    List<RelatedRelationEntity> relatedRelation2 = (from g in G.relationList
                                                                    let x = SemanticSearch_DTCDTNN.BetaDatabase.FirstOrDefault(b => b.FirstRelationId == g.idRelation && b.SecondRelationId == H.relationList[i].idRelation)
                                                                    where x != null
                                                                    select new RelatedRelationEntity
                                                                    {
                                                                        relation = g,
                    ...
                    for (int j = 0; j < relatedRelation.Count; j++)
                    {
                        ...
                        {
                            alpha1 = (from a in SemanticSearch_DTCDTNN.AlphaDatabase
                        ...
                        {
                            alpha2 = (from a in SemanticSearch_DTCDTNN.AlphaDatabase
        ...
        float? GetAlphaSynonym(KeyphraseEntity k1, KeyphraseEntity k2)
        {
            if (k1.id == k2.id)
                return 1;
            float? alpha = (from a in SemanticSearch_DTCDTNN.AlphaDatabase
