...
namespace CreateKeyphraseGraphLibrary.Helper
{
    public class EstablishKeyphrase
    {
        ...
        {
            var formOrderEntity = KeyphraseGraph.Keyphrases.Where(p => String.Compare(p.Name, keyphrase) == 0).FirstOrDefault();
                ...
                if (formOrderEntity.MonoKeyphrases != null)
                {
                    ...
                    if (estabElements.Length == 1)
                    {
                        ...
                        //if (!strEstabRelation.Contains(aRelation))
                            ...
                            GetEstablish_KeyphraseAndRelation_XEMLAI(estabElements[0], ref estabKeyphrases, ref strEstabRelation);
                    }
                    else //estabElements.Length == 2
                    {
                        ...
                        //if (!strEstabRelation.Contains(aRelation))
                            strEstabRelation.Add(aRelation);
                            GetEstablish_KeyphraseAndRelation_XEMLAI(estabElements[0], ref estabKeyphrases, ref strEstabRelation);
                            GetEstablish_KeyphraseAndRelation_XEMLAI(estabElements[1], ref estabKeyphrases, ref strEstabRelation);
                    }
                }
                else
        ...
        public static void GetEstablish_KeyphraseAndRelation(string keyphrase, ref List<NameEntity> estabKeyphrases, string monoKeyphrases)
        {
            ...
            if (estabElements.Length == 1)
            {
                ...
                var entity = KeyphraseGraph.Keyphrases.Where(p => String.Compare(p.Name, strTemp) == 0).FirstOrDefault();
                if (entity != null)
                {
            ...
            else //estabElements.Length == 2
            {
                ...
                var entity1 = KeyphraseGraph.Keyphrases.Where(p => String.Compare(p.Name, strTemp1) == 0).FirstOrDefault();
                if (entity1 != null)
                {
