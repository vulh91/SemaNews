...
{
    public class KeyphraseGraph_DTCDTNN
    {
        static DTC_DTNN_CKONTOLOGYCreateKGEntities dbCKOntology;
        ...
        {
            using (dbCKOntology = new DTC_DTNN_CKONTOLOGYCreateKGEntities())
            {
        ...
        {
            List<NumberKeyphraseEntity> titleList = new List<NumberKeyphraseEntity>();
            List<NumberKeyphraseEntity> abstractList = new List<NumberKeyphraseEntity>();
            List<NumberKeyphraseEntity> contentList = new List<NumberKeyphraseEntity>();
            List<NumberKeyphraseEntity> tagsList = new List<NumberKeyphraseEntity>();
            ...
            foreach (Keyphrase k in Keyphrases)
            {
                ...
                occurrenceNumber = Regex.Matches(_title, k.Name).Count;
                if (occurrenceNumber > 0)
                    titleList.Add(new NumberKeyphraseEntity { Id = k.Id, keyphrase = k.Name, number = occurrenceNumber });
                occurrenceNumber = Regex.Matches(_abstract, k.Name).Count;
                if (occurrenceNumber > 0)
                    abstractList.Add(new NumberKeyphraseEntity { Id = k.Id, keyphrase = k.Name, number = occurrenceNumber });
                occurrenceNumber = Regex.Matches(_content, k.Name).Count;
                if (occurrenceNumber > 0)
                    contentList.Add(new NumberKeyphraseEntity { Id = k.Id, keyphrase = k.Name, number = occurrenceNumber });
                occurrenceNumber = Regex.Matches(_tags, k.Name).Count;
                if (occurrenceNumber > 0)
                    tagsList.Add(new NumberKeyphraseEntity { Id = k.Id, keyphrase = k.Name, number = occurrenceNumber });
            }
